...
export default function Operacao({ chamado, onClick }) {
    ...
    return (
        <section
            className={`${styles.caixaOperacao} ${styles[classePrioridade]}`}
            onClick={onClick} // Executa a função recebida no clique
        >
            <div className={styles.borderStatus}></div>
            <div>
                <div className={styles.status}>
                    <p className={styles.servicoRealizado}>#{chamado.codigo} • {chamado.status.toUpperCase()}{chamado.status === "Finalizado" ? `: ${chamado.servico_realizado}` : ''}</p>
                    <p className={
